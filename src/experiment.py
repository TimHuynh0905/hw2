...
def run(data_path, learner_type, fraction):
    ...
    train_class = DecisionTree(attribute_names) if learner_type == "decision_tree" else PriorProbability()
    
    train_features, train_targets, test_features, test_targets = train_test_split(features, targets, fraction)

    train_class.fit(train_features, train_targets)
    
    predictions = train_class.predict(test_features)
